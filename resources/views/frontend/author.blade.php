...
<x-navbar />
        <nav id="Category" class="max-w-[1130px] mx-auto flex justify-center items-center gap-4 mt-[30px]">
            @foreach($categories as $item_category)
                <a href="{{ route('frontend.category', $item_category->slug) }}" class="rounded-full p-[12px_22px] flex gap-[10px] font-semibold transition-all duration-300 border border-[#EEF0F7] hover:ring-2 hover:ring-[#FF6B18]">
    ...
    <section id="author" class="max-w-[1130px] mx-auto flex items-center flex-col gap-[30px] mt-[70px]">
		...
		@forelse($author->news as $news)
            <a href="{{route('frontend.details', $news->slug)}}" class="card">
                <div
        ...
        <div class="flex justify-center">
            <a href=""
                class="rounded-full p-[12px_22px] flex gap-[10px] font-semibold transition-all duration-300 bg-[#FF6B18] text-white hover:shadow-[0_10px_20px_0_#FF6B1880]">Show
                All</a>
        </div>
    </section>
    <section id="related-news" class="max-w-[1130px] mx-auto flex flex-col gap-[30px] mt-[70px]">
        <div id="title" class="flex items-center gap-[30px]">
            <h1 class="text-4xl leading-[45px] font-bold">Related News</h1>
        </div>
        <div id="content-cards" class="grid grid-cols-2 gap-[30px]">
			@forelse($author->news as $news)
            <a href="{{ route('frontend.details', $news->slug) }}" class="card">
                <div
                    class="flex flex-col gap-4 p-[26px_20px] transition-all duration-300 ring-1 ring-[#EEF0F7] hover:ring-2 hover:ring-[#FF6B18] rounded-[20px] overflow-hidden bg-white">
			...
			@empty
			<p>Belum ada artikel yang di terbitkan</p>
			@endforelse
